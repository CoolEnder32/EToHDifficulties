    ...
    bool init(GJGameLevel* level, bool challenge) {
        ...
        if (!demon && !level->m_autoLevel) {
            if      (stars == 2)               { rangeText = "1-1.99";    subText = "Easy";                                 subScale = 0.45f; iconKey = "ez";          rightAlign = true; }
                ...
                iconKey = "hard"; useStacked = true;
                stackNum1 = "3";    stackCol1 = getDifficultyColor("Hard");
                stackNum2 = "3.5";  stackCol2 = getDifficultyColor("Hard");
            }
            else if (stars == 6 || stars == 7) {
                iconKey = "hard"; useStacked = true; iconXAdjust = 5.f;
                stackNum1 = "3.5";   stackCol1 = getDifficultyColor("Hard");
                stackNum2 = "3.99";  stackCol2 = getDifficultyColor("Hard");
            }
        ...
        if (!iconKey.empty()) {
            {
                auto frameName = Mod::get()->expandSpriteName((iconKey + ".png").c_str());
                auto iconSpr = CCSprite::createWithSpriteFrameName(frameName.c_str());
                if (iconSpr) {
                    CCSize diffSize  = diffSprite->getContentSize();
                    float  diffScale = diffSprite->getScale();
                    float  targetH   = diffSize.height * diffScale;
                    bool   isEzMed   = (iconKey == "ez" || iconKey == "med" || iconKey == "difftochall" || iconKey == "hard");
